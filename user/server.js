...
app.post('/login', (req, res) => {
    const body = req.body || {};
    const name = body.name || body.username || req.body?.name || req.query?.name;
    const password = body.password || req.body?.password || req.query?.password;
    req.log.info('login', { name, password: password ? '[redacted]' : undefined });
...
app.post('/register', (req, res) => {
    ...
    req.log.info('register', { name, email, password: password ? '[redacted]' : undefined });
    if(!name || !password || !email) {
        req.log.warn('insufficient data');
        res.status(400).send('insufficient data');
    } else if(mongoConnected) {
        // check if name already exists
        usersCollection.findOne({name: name}).then((user) => {
            if(user) {
